...
| Compile a resume (select → render → gate) | POST | `/api/v1/compile` | implemented | Selector chooses under the budget, LLM phrases, linker and entailment gate reject anything stronger than its evidence. Persists only documents that passed both gates. |
